...
export default function FAQContent({ faqData }: { faqData: FAQData | null }) {
  ...
  const categories = faqData.categories ||
    [...new Set(faqData.questions?.map((q) => q.category).filter(Boolean) as string[])];
          ...
          <Card className="mb-8">
            <CardContent className="pt-6">
              <Accordion type="single" collapsible className="w-full">
                {filteredQuestions.map((faq, index: number) => (
