...
import { ArrowUpDown, Eye, CreditCard as Edit, FileImage, Calendar } from "lucide-react";
...
export const columns: ColumnDef<LineAd>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const holdComment = comments?.find(
        (comment: any) => comment.actionType === "HOLD"
      );

      if (!holdComment?.comment) {
        return (
      ...
      return (
        <div className="max-w-xs">
          <p
            className="text-sm text-orange-600 truncate"
            title={holdComment.comment}
          >
            {holdComment.comment}
          </p>
        </div>
