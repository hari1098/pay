...
import { User, Customer } from '../../../shared/models/models';
...
@Component({
  ...
  template: `
    <div class="space-y-6 max-w-4xl mx-auto p-6">
      ...
      @if (isLoading()) {
        <div class="space-y-6"><div class="h-[300px] bg-gray-100 rounded animate-pulse"></div></div>
      } @else {
        <div class="bg-white rounded-lg border shadow-sm p-6">
          ...
          <form [formGroup]="profileForm" (ngSubmit)="onProfileSubmit()" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label class="block text-sm font-medium mb-1">Phone</label><input type="tel" formControlName="phone_number" class="h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" /></div>
            </div>
...
export class DashboardProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (user) {
      this.profileForm.patchValue({ name: user.name, email: user.email, phone_number: user.phone_number });
    }
  ...
  onProfileSubmit() {
    ...
    this.api.updateProfile(this.profileForm.value).subscribe({
      next: () => { this.isSaving.set(false); this.notification.success('Profile updated successfully'); },
  ...
  onPasswordSubmit() {
    ...
    this.api.changePassword(this.passwordForm.value).subscribe({
      next: () => { this.isChangingPassword.set(false); this.notification.success('Password changed successfully'); this.passwordForm.reset(); },
