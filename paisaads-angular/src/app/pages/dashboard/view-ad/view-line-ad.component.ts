...
@Component({
  selector: 'app-view-line-ad',
  standalone: true,
  imports: [RouterLink],
  template: `
    <div class="space-y-6 max-w-3xl mx-auto">
      <div class="flex items-center gap-4">
        <a routerLink="/dashboard/my-ads/line-ads" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back to My Ads</a>
      </div>
      @if (isLoading()) {
        ...
        <div class="bg-white rounded-lg border shadow-sm p-6 space-y-4">
          <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Line Ad Details</h1>
          ...
          <div>
            ...
            <div class="p-4 bg-gray-50 rounded-lg" [style.backgroundColor]="ad()!.backgroundColor || '#fff'" [style.color]="ad()!.textColor || '#000'">
              {{ ad()!.content }}
            </div>
          </div>
            ...
            <div>
              ...
              <div class="flex gap-2 flex-wrap">
                @for (img of ad()!.images; track img.id) {
                  <img [src]="getImageUrl(img.fileName)" class="w-24 h-24 object-cover rounded border" />
                }
              </div>
            </div>
          }
...
export class ViewLineAdComponent implements OnInit {
  private api = inject(ApiService);
  private route = inject(ActivatedRoute);
  ad = signal<LineAd | null>(null);
  isLoading = signal(true);

  ngOnInit() {
    const id = this.route.snapshot.paramMap.get('id') || '';
    this.api.getLineAdById(id).subscribe({
