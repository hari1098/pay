...
@Component({
  selector: 'app-view-video-ad',
  standalone: true,
  imports: [RouterLink],
  template: `
    <div class="space-y-6 max-w-3xl mx-auto">
      <div class="flex items-center gap-4">
        <a routerLink="/dashboard/my-ads/video-ads" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back to My Ads</a>
      </div>
      @if (isLoading()) {
        ...
        <div class="bg-white rounded-lg border shadow-sm p-6 space-y-4">
          <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold">Video Ad Details</h1>
          ...
          @if (ad()!.videoUrl) {
            <div>
              ...
              <video controls class="w-full max-w-lg rounded border" [src]="ad()!.videoUrl"></video>
            </div>
          }
        </div>
      }
...
export class ViewVideoAdComponent implements OnInit {
  private api = inject(ApiService);
  private route = inject(ActivatedRoute);
  ad = signal<VideoAd | null>(null);
  isLoading = signal(true);

  ngOnInit() {
    const id = this.route.snapshot.paramMap.get('id') || '';
    this.api.getVideoAdById(id).subscribe({
